...
for i=7:endtime/dt
    ...
    for j=4:(rgrid(end)-2*M)/dx -3
        h(6,j)=-4/15 * ( (-77/6+5/2*p-dt^2*VRWpot(j))*h(5,j)...
            +107/6*h(4,j) -13*h(3,j) +61/12*h(2,j) -5/6*h(1,j) ...
            -p*(-1/12*h(5,j-2) +4/3*h(5,j-1)...
            +4/3*h(5,j+1) -1/12*h(5,j+2)));
      
    end
        for j=2:3
        h(8,j)=-4/15 * ( (-223/(10)+49*dt^2/(18*dx^2)-dt^2*VRWpot(j))*h(7,j)...
